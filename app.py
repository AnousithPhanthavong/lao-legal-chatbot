...
LEGAL_TERMS = [
    ...
    'ຄຸ້ມຄອງ', 'ຕິດຕາມ', 'ກວດກາ', 'ຕ່າງປະເທດ', 'ພາສີ',
    'ອາກອນ', 'ຫຼັກຊັບ', 'ທະນາຄານ', 'ປ່າໄມ້', 'ທີ່ດິນ',
    'ສິ່ງແວດລ້ອມ', 'ແຮງງານ', 'ອາຍາ', 'ແພ່ງ', 'ເຕັກໂນໂລຊີ',
    'ຂົນສົ່ງ', 'ກະສິກຳ',
]
...
def load_system():
    ...
    if not api_keys:
        st.error('No API keys found.'); st.stop()
...
DEFINITIONAL = ['ແມ່ນຫຍັງ', 'ຄືແນວໃດ', 'ໝາຍຄວາມວ່າ', 'ຈຸດປະສົງ', 'ຄຳນິຍາມ', 'ຂອບເຂດ', 'ອະທິບາຍ']
...
def search(query, sys):
    ...
    results = []
    seen_ids = set()
    is_definitional = any(p in query for p in DEFINITIONAL)
    if article_num:
        ...
        for i in range(len(cr['ids'][0])):
            ...
            if cid not in seen_ids:
                ...
                if chunk:
                    ...
                    seen_ids.add(cid)
        if law_filter:
            cr2 = sys['collection'].query(
                query_embeddings=[q_vec], n_results=5, where=None,
                include=["documents","metadatas","distances"])
            for i in range(len(cr2['ids'][0])):
                cid = cr2['ids'][0][i]
                if cid not in seen_ids:
                    sim = 1 - cr2['distances'][0][i]
                    chunk = sys['chunks_by_id'].get(cid)
                    if chunk:
                        results.append({'chunk': chunk, 'score': sim * 0.9, 'source': 'semantic'})
                        seen_ids.add(cid)
    except: pass
    try:
    ...
    if is_definitional and law_filter:
        for chunk in sys['all_chunks']:
            ...
            if (meta.get('article', 0) <= 5
                ...
                results.append({'chunk': chunk, 'score': 0.9 - (meta['article'] * 0.02), 'source': 'definitional'})
                seen_ids.add(chunk['id'])
...
SYSTEM_PROMPT = (
    ...
    "- ຢ່າອ້າງວ່າມີປະສົບການ ຫຼື ເປັນທະນາຍ — ເຈົ້າແມ່ນເຄື່ອງມືຄົ້ນຄວ້າ.\n"
)
...
def generate_answer(query, search_results, confidence, sys):
    ...
    conf_note = ""
    if confidence == 'LOW':
        conf_note = "\nໝາຍເຫດ: ຜົນຄົ້ນຫາບໍ່ກົງກັບຄຳຖາມຫຼາຍ. ຕອບຈາກສິ່ງທີ່ມີ."
    ...
    with st.chat_message('assistant'):
        if len(user_input.strip()) < 3:
            ...
            if cached:
                ...
                from_cache = True; citations_ok = cached.get('citations_ok',True)
            ...
            if show_debug and not from_cache:
                with st.expander("🔧 Debug", expanded=False):
                    st.json({'search_time':f"{t_search:.2f}s",'gen_time':f"{t_gen:.2f}s",
                        ...
                        'article_num':article_num,'results':len(search_results)})
